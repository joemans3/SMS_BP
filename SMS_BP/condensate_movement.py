...
class Condensate:
    ...
    def add_positions(self, time: np.ndarray, position: np.ndarray, scale: np.ndarray) -> None:
    ...
    def _generate_condensate_positions(self, time: int) -> None:
        ...
        t, xy = fbm.get_fbm_sample(l=time_difference, n=int(
            time_difference)+1, h=self.hurst_exponent, d=self.dim)
        # convert the xy into [[x,y],...] format
        x, y = xy
        coords = np.stack((x, y), axis=-1) * \
            np.sqrt(2*self.diffusion_coefficient)
    ...
    def plot_condensate(self, ax: plt.Axes, **kwargs) -> plt.Axes:
        ...
        **kwargs:
        ...
        ax.plot(self.condensate_positions[:, 0],
                self.condensate_positions[:, 1], **kwargs)
        ...
        ax.scatter(self.initial_position[0],
                   self.initial_position[1], color='g')
        # plot the final position in a different colour
        ax.scatter(
            self.condensate_positions[-1][0], self.condensate_positions[-1][1], color='b')
