...
class Condensate:
    ...
    def condensate_positions(self, value)->None:
        ...
        if value.shape[1] != self.dim:
            raise ValueError('Condensate positions must be the same dimension as the initial position.')
    ...
    def add_positions(self, time: np.ndarray, position: np.ndarray, scale: np.ndarray)->None:
        ...
        self.condensate_positions = np.append(self.condensate_positions, position, axis=0)
    ...
    def _generate_condensate_positions(self, time: int)->None:
        ...
        coords = coords + self.condensate_positions[-1] #the first position is the last position in the condensate_positions so ignore it when adding to the condensate_positions
