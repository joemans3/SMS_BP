...
class FBM_BP:
    ...
    def fbm(self):
        ...
        gn = np.random.normal(0, 1, self.n) * \
            np.sqrt(self.dt*2*self._diff_a_n)*(self.dt**(2*self._hurst_n))
...
def _boundary_conditions(fbm_store_last: float, fbm_candidate: float, space_lim: np.ndarray, condition_type: str):
    ...
    if condition_type not in BOUNDARY_CONDITIONS:
        raise ValueError('Invalid condition type: ' + condition_type +
                         "! Must be one of: " + str(BOUNDARY_CONDITIONS.keys()))
...
if __name__ == "__main__":
    ...
    plt.bar(possible_states, state_probability,
            label='State probability distribution', alpha=0.9)
    ...
    print('Probability of switching from state 1 to state 2: ' +
    ...
    print('Probability of switching from state 2 to state 1: ' +
