...
def sub_segment(img, sub_frame_num, img_name=None, subsegment_type="mean"):
    ...
    for j in np.arange(sub_frame_num):
        if subsegment_type == "mean":
            hold_img.append(np.mean(
                img[int(j*frames_per_subsegment):int((j+1)*frames_per_subsegment)], axis=0))
        elif subsegment_type == "max":
            hold_img.append(np.max(
                img[int(j*frames_per_subsegment):int((j+1)*frames_per_subsegment)], axis=0))
        elif subsegment_type == "std":
            hold_img.append(np.std(
                img[int(j*frames_per_subsegment):int((j+1)*frames_per_subsegment)], axis=0))
    return hold_img
...
def make_directory_structure(cd, img_name, img, subsegment_type, sub_frame_num, **kwargs):
    ...
    **kwargs : dict
    ...
    for i in np.arange(sub_frame_num):
        hold_name.append(os.path.join(
            cd, "segmented", str(int(i)+1)+"_"+img_name+".tif"))
...
class Simulate_cells():
    def __init__(self, init_dict_json: dict | str):
        ...
        self.track_length_mean = self._convert_frame_to_time(
            self.init_dict["Track_Parameters"]["track_length_mean"], self.exposure_time, self.interval_time, self.oversample_motion_time)

        # update the diffusion coefficients from um^2/s to pix^2/ms
        self.track_diffusion_updated = self._update_units(
            self.init_dict["Track_Parameters"]["diffusion_coefficient"], 'um^2/s', 'pix^2/(oversample_motion_time)ms)')
        self.condensate_diffusion_updated = self._update_units(
            self.init_dict["Condensate_Parameters"]["diffusion_coefficient"], 'um^2/s', 'pix^2/(oversample_motion_time)ms)')
    ...
    def _convert_track_dict_points_per_frame(self, tracks, movie_frames):
        ...
        '''
        points_per_frame = dict(zip([str(i) for i in range(movie_frames)], [
    ...
    def _create_track_pop_dict(self, simulation_cube: np.ndarray):
        ...
        self.condensates = sf.create_condensate_dict(
            ...
            units_time=np.array([str(self.init_dict["Global_Parameters"]["oversample_motion_time"]) +
                                self.init_dict["time_unit"]]*len(self.condensate_diffusion_updated)),
        ...
        initials = np.zeros(
            (self.init_dict["Track_Parameters"]["num_tracks"], 3))
        ...
        for i in range(self.init_dict["Track_Parameters"]["num_tracks"]):
            ...
            for ID, cond in self.condensates.items():
                condensate_positions[int(ID)] = cond(int(starting_frame), str(
                    self.init_dict["Global_Parameters"]["oversample_motion_time"])+self.init_dict["time_unit"])["Position"]
        ...
        if initials.shape[1] == 2:
            ...
            initials = np.hstack(
                (initials, np.zeros((self.init_dict["Track_Parameters"]["num_tracks"], 1))))
        # create tracks
        tracks = {}
        points_per_time = dict(zip([str(i) for i in range(int(self.total_time))], [
        ...
        if self.init_dict["Track_Parameters"]["track_type"] == "constant":
            ...
            track_diffusion_choice = np.random.choice(
                index_diffusion, size=self.init_dict["Track_Parameters"]["num_tracks"], p=self.init_dict["Track_Parameters"]["diffusion_track_amount"])
            track_hurst_choice = np.random.choice(
                index_hurst, size=self.init_dict["Track_Parameters"]["num_tracks"], p=self.init_dict["Track_Parameters"]["hurst_track_amount"])
            for i in range(self.init_dict["Track_Parameters"]["num_tracks"]):
    ...
    def _create_map(self, initial_map: np.ndarray, points_per_frame: dict, axial_function: str):
        ...
        for i in range(initial_map.shape[0]):
            ...
            if len(points_per_frame[str(i)]) == 0:
                abs_axial_position = 1.0 * \
                    self.init_dict["Global_Parameters"]["point_intensity"] * \
                    self.oversample_motion_time/self.exposure_time
    ...
    def _point_per_time_selection(self, points_per_time: dict) -> dict:
        ...
        points_per_frame = dict(zip([str(i) for i in range(self.frame_count)], [
